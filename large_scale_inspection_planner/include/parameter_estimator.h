...
public:
    ...
    const std::vector< std::vector< std::vector<float> > >& getTimeCostMatrices();
    const std::vector< std::vector< std::vector<float> > >& getBatteryDropMatrices();
...
private:
    ...
    std::vector< std::vector< std::vector<float> > > time_cost_matrices_;     // One square symetrical matrix for each UAV. The elements are the time (in seconds) to cover that edge, if the edge doesn't make sense or there is no connection possible then the value is -1.
    std::vector< std::vector< std::vector<float> > > battery_drop_matrices_;  // One square non-symetrical matrix for each UAV. The elements are the battery drop (per unit, not %) to cover that edge, if the edge doesn't make sense or there is no connection possible then the value is -1.
