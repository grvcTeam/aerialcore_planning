...
private:
    ...
    std::vector< std::vector< std::vector<float> > > time_cost_matrices_v_;
    std::vector< std::vector< std::vector<float> > > battery_drop_matrices_v_;
    ...
    // VNS heuristic:
    float solutionTimeCostTotal(const std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    float solutionTimeCostMaximum(const std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    bool solutionValidOrNot(const std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    void shake(std::vector< std::vector< std::pair<int,int> > >& _inspection_edges, std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    void localSearch(std::vector< std::vector< std::pair<int,int> > >& _inspection_edges, std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    ...
    std::vector< std::vector< std::pair<int,int> > > addNavigationsToInspectionEdges(const std::vector< std::vector< std::pair<int,int> > >& _inspection_edges);
    void buildPlanNodesFromInspectionEdges(const std::vector< std::vector< std::pair<int,int> > >& _solution_edges);
    void precomputeClosestLandStations();
    void mapsToVectors();   // For the shake() and localSearch() no maps will be used as they slow down the performance.
    std::vector<int> closest_land_station_;           // Size of graph_, closest land station to each of the graph nodes (-1 if no sense).
    std::vector<int> id_inspection_edges_;            // Same size as the inspection edges, UAV id for each of the vector of pairs of inspection edges.
    std::vector<int> initial_node_inspection_edges_;  // Same size as the inspection edges, returns the graph index of the initial UAV position node for each of the vector of pairs of inspection edges.
    std::vector<int> UAVs_index_inspection_edges_;    // Same size as the inspection edges, returns index in the UAVs vector for each of the vector of pairs of inspection edges.
