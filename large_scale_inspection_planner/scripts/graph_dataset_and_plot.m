...
# "pylons_position_cartesian" describes the (x,y,z) points of the electric towers (or pylons) of the complete electric graph, being the three columns the (x,y,z) coordinates and the rows different pylon elements.
# "pylons_position_geographic" is just the same but in geographic coordinates (latitude, longitude, altitude).
# The vector of vectors "pylons_connections_indexes" must have the same number of rows as "pylons_position_cartesian" (and "pylons_position_geographic") and represents the wire connections among electric towers.
# ATTENTION: indexes of "pylons_connections_indexes" start at 1, so you may want to rest 1 at parsing to start by 0 (C++ or Python).
...
# Plot the graph:
hold on
[N_pylons,~] = size(pylons_connections_indexes);
for it_1=1:N_pylons
  ...
  for it_2=1:N_connections_current_pylon
    if it_1 > pylons_connections_indexes{it_1}(1,it_2)  # Don't repeat twice the connection
      x=[pylons_position_cartesian(it_1,1) pylons_position_cartesian(pylons_connections_indexes{it_1}(1,it_2),1)];
      y=[pylons_position_cartesian(it_1,2) pylons_position_cartesian(pylons_connections_indexes{it_1}(1,it_2),2)];
      plot(x, y, '-r');
      text (x(1,1), y(1,1), mat2str(it_1));
      text (x(1,2), y(1,2), mat2str(pylons_connections_indexes{it_1}(1,it_2)));
    end
  end
end
hold off
